...
{	static private $failures;	
	...
	{	if(false === $trueThing)
		...
		$lines = self::getFunctionCallLines($backTrace[0]['file'], "assert", $backTrace[0]["line"]);
		$linesDisplay = "'<i>".implode("<br>\n",$lines)."</i>'";
		if(count($lines) > 1)
		{	$linesDisplay = "<br>\n".$linesDisplay;
		}
		
		echo '<div><span style="color:'.$color.';">'.$word.':</span> '.$backTrace[1]["class"]." :: ".$backTrace[1]["function"].
				" at [".basename($backTrace[0]["file"])." line ".$backTrace[0]["line"]."] ".$linesDisplay."</div>";
	}

    // takes in a list of class names and makes their methods public
    public static function publicifyClassMethods(/* varargs */)
			...
			{	self::$failures = 0;
				...
				echo '<div id="unitTest_'.get_called_class().$n.'" style="margin:1px;">';
					...
					echo '<div id="unitTest_'.get_called_class().$n.'_inner">';
						...
						{	echo '<span style="color:red;">Exception:</span> '.get_called_class()." :: ".$f."<br>";
							echo $e;
